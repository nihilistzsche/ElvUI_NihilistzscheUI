...
function PBN:AddStyleFilterOptions()
	...
	E.Options.args.nameplate.args.filters.args.selectFilter.set = function(info, value)
		oldFunc(info, value)
		local tbl = E.Options.args.nameplate.args.filters.args.triggers.args.combat.args
		if not tbl.isBattlePetPBN then
			local order = 53
			local PROVIDED_BY_NUI = "  This filter is provided by NihilistUI."
			...
			E.Options.args.nameplate.args.filters.args.triggers.args.instanceType.args.petBattle = {
				name = "Pet Battle",
				type = "toggle",
				order = 20
			}
...
function PBN:GenerateOptions()
	...
	local options = {
		...
		get = function(info)
			return E.db.nihilistui.petbattlenameplates[info[#info]]
		end,
		set = function(info, value)
			E.db.nihilistui.petbattlenameplates[info[#info]] = value
		end,
		args = {
			header = {
				order = 1,
