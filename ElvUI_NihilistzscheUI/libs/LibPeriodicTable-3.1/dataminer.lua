...
local function printdiff(set, old, new)
    ...
    local has_drop_rate = set:find("InstanceLoot", nil, true) and not set:find("Trash Mobs", nil, true)
    local temp = {}
    local oldEntries = 0
    local newEntries = 0
    if old then
        for entry in old:gmatch("[^,]+") do
            ...
            if has_drop_rate then
                entry = entry:match("(%d+):%d+")
            end
...
do
    ...
    elseif (URL_TRANSPORT == TRANSPORT_TYPES.CURL_SHELL) then
        function getpage(p_url)
            if DEBUG_TRACK_URL then
                dprint(1, "curl shell", p_url)
            end
            local start_time = os.time()
            local shell_command =
                'curl --silent --show-error --user-agent "Mozilla/5.0" --insecure --location "' .. p_url .. '"'
            --print(shell_command)
...
CREATE TABLE IF NOT EXISTS cache (
	...
	PRIMARY KEY (url)
        ...
        local select_stmt =
            db:prepare(
            "SELECT content FROM cache WHERE url = ? AND datetime(time, '" .. CACHE_TIMEOUT .. "') > datetime('now')"
        )
...
local function read_data_file()
    ...
    if (arg[1] or ""):match("^InstanceLoot%.") then
        for set, data in file:gmatch(
            '\t%[%"(' .. (string.gsub(subset, "^InstanceLoot", "InstanceLootHeroic")) .. '[^"]*)%"%][^=]-= "([^"]-)"'
        ) do
            sets[set] = data
            setcount = setcount + 1
        end
        for set, data in file:gmatch(
            '\t%[%"(' .. (string.gsub(subset, "^InstanceLoot", "InstanceLootLFR")) .. '[^"]*)%"%][^=]-= "([^"]-)"'
        ) do
            sets[set] = data
            setcount = setcount + 1
        end
...
local function basic_listview_get_npc_id(npc, zone)
    ...
    if npc == "Sathrovarr the Corruptor" then
        return 24892
    end
    if npc == "Electron" then
        return 42179
    end
    -- override because "," in names screws things up
    if npc == "Corla Herald of Twilight" then
        return 39679
    end
    -- override because NPC name exists multiple times
    if npc == "Kael'thas Sunstrider" and zone == "Magisters' Terrace" then
        return 24664
    end
    local url = WH("npcs", nil, {na = npc, cr = {32, 9}, crs = {1, 1}, crv = {0, 0}, ma = 1})
    local views = get_page_listviews(url)
    if not views.npcs then
        return
    end
    local data = views.npcs.data
    if zone then
        zone = get_zone_id_from_name(zone)
    end
    local first_id
...
local Tradeskill_Tool_filters = {
    ...
    Blacksmithing = {
        {cr = 91, crs = 162, crv = 0},
        -- Tool - Blacksmith Hammer
        {cr = 91, crs = 161, crv = 0},
        -- Tool - Gnomish Army Knife
        {cr = 91, crs = 167, crv = 0}
        -- Tool - Hammer Pick
    },
    --
	...
	Cooking = {
        ...
        {cr = 91, crs = 190, crv = 0},
        -- Tool - Runed Titanium Rod [ckaotik]
...
local function handle_trash_mobs(set)
    ...
    for _, level in ipairs(info.levels) do
        ...
        local url =
            WH("items", nil, {minle = level, maxle = level, cr = cr, crs = crs, crv = crv, qu = {3, 4, 5, 6, 7}})
        local set =
            basic_listview_handler(
            url,
            function(item)
...
do
    ...
    is_junk_drop = function(itemid)
        ...
        basic_listview_handler(
            url,
            function(entry)
                if entry.count and entry.count < INSTANCELOOT_MIN then
                    return
                end
                count = count + 1
            end,
...
handlers["^ClassSpell"] = function(set, data)
    ...
    return basic_listview_handler(
        WH("spells", Class_Skills_categories[class][tree]),
        function(item)
...
handlers["^Consumable%.Buff Type"] = function(set, data)
    ...
    if setname ~= "Both" and not filter then
        return
    end
...
local Consumable_Potion_Recovery_filters = {
    ...
    ["Healing.Basic"] = {
        ...
        crs = "0:0",
        crv = "restores:health",
        excludes = {"Healing.Zone-Restricted", "Healing.Endless", "Rejuvenation.Basic", "Rejuvenation.Trance"}
    },
    ...
    ["Mana.Basic"] = {
        ...
        crs = "0:0",
        crv = "restores:mana",
        excludes = {"Mana.Zone-Restricted", "Mana.Endless", "Rejuvenation.Basic", "Rejuvenation.Trance"}
    }
}
...
handlers["^Consumable%.Potion%.Recovery"] = function(set, data)
    ...
    return basic_listview_handler(
        WH("items", "0.1", Consumable_Potion_Recovery_filters[setName]),
        function(item)
...
handlers["^CurrencyItems"] = function(set, data)
    ...
    if currency_id > 0 then
        ...
        return basic_listview_handler(
            WH("currency", -currency_id),
            function(item)
                local count
                for _, v in ipairs(item.cost[2]) do
                    if v[1] == 396 and item.level == 359 then
                        return
                    end --XXX temp fix for jp items showing up on a vp vendor (http://www.wowhead.com/npc=44245)
                    if v[1] == -currency_id then
                        count = v[2]
                        break
                    end
...
handlers["^InstanceLoot%."] = function(set, data)
    if not INSTANCELOOT_CHKSRC then
        return
    end
    local newset = {}
    ...
    if id then
        ...
        local handler = function(item)
            ...
            if is_heroic then
                handle_heroic_item(item)
            end
            if is_lfr then
                handle_lfr_item(item)
            end
            ...
            local totalDrops =
                (item.modes and item.modes["2"] and item.modes["2"].count) or
                (item.modes and item.modes["8"] and item.modes["8"].count) or
                (item.modes and item.modes["16"] and item.modes["16"].count) or
                item.count or
                0
            local totalLoot =
                (item.modes and item.modes["2"] and item.modes["2"].outof) or
                (item.modes and item.modes["8"] and item.modes["8"].outof) or
                (item.modes and item.modes["16"] and item.modes["16"].outof) or
                totaldrops or
                0
        ...
        dprint(
            ...
            "InstanceLoot: " ..
                boss ..
                    " has " ..
                        (newset and #newset or 0) ..
                            " normal, " .. count_heroic .. " heroic and " .. count_lfr .. " raid finder drops."
        )
...
handlers["^InstanceLootHeroic%."] = function(set, data)
    ...
    if id then
        ...
        local handler = function(item)
            ...
            local is_heroic = is_heroic_item(item)
            if not is_heroic then
                return
            end

            local totalDrops =
                (item.modes["32"] and item.modes["32"].count or 0) + (item.modes["64"] and item.modes["64"].count or 0) +
                (item.modes["1"] and item.modes["1"].count or 0)
            local totalLoot =
                (item.modes["32"] and item.modes["32"].outof or 0) + (item.modes["64"] and item.modes["64"].outof or 0) +
...
handlers["^Misc%.Lockboxes"] = function(set, data)
    return basic_listview_handler(
        WH("items", nil, {cr = 10, crs = 1, crv = 0}),
        function(item)
...
handlers["^Consumable.Food.Edible.Combo.Conjured"] = function(set, data)
    return basic_listview_handler(
        WH("items", "0.5", {cr = "9:107:107", crs = "1:0:0", crv = "0:health:mana"}),
        function(item)
            page = getpage(WH("item", item.id) .. "&xml") -- hack
...
handlers["^Misc%.Mount%."] = function(set, data)
    ...
    return basic_listview_handler(
        WH("items", category, {cr = 161, crs = 1, crv = 0}),
        function(item)
            return item.id .. ":" .. item.level
        end
    )
...
handlers["^Tradeskill%.Crafted"] = function(set, data)
    ...
    printdiff(
        "TradeskillResultMats.Forward." .. profession,
        sets["TradeskillResultMats.Forward." .. profession],
        fp_set
    )
    ...
    printdiff(
        "TradeskillResultMats.Reverse." .. profession,
        sets["TradeskillResultMats.Reverse." .. profession],
        rp_set
    )
...
handlers["^Tradeskill%.Gather%."] = function(set, data)
    ...
    if set:match("^Tradeskill%.Gather%.GemsInNodes") then
        ...
        return basic_listview_handler(
            WH("object", id),
            function(item)
                if item.classs == 7 and item.subclass == 4 then
                    return item.id
                end
            end,
            "mining"
...
handlers["^Tradeskill%.Recipe%."] = function(set, data)
    ...
    if not profession or not filter then
        return
    end

    return basic_listview_handler(
        WH("items", profession, filter),
        function(item)
...
local function write_output(file, sets)
    ...
    for line in file:gmatch("([^\n]-\n)") do
        ...
        if setname then
            ...
            if
                (DEBUG > 0) and
                    not (string.match(line, comment_ptn) or string.match(line, block_end_ptn) or
                        string.match(line, blank_ptn))
             then --A weird line, possibly an error
