...
local function skip_space(s, startPos)
    if DEBUG then
        print("skip_space", startPos, string_sub(s, startPos))
    end
...
local function decode_string(s, startPos)
    if DEBUG then
        print("decode_string", startPos, string_sub(s, startPos))
    end
    local escaped = false
    ...
    while true do
        ...
        elseif char == quote_type and not escaped then
            if DEBUG then
                print("in decode_string", string_sub(s, startPos, endPos))
            end
...
local function decode_constant(s, startPos, allowStringConstants)
    if DEBUG then
        print("decode_constant", startPos, allowStringConstants, string_sub(s, startPos))
    end
...
local function decode_list(s, startPos)
    if DEBUG then
        print("decode_list", startPos, string_sub(s, startPos))
    end
    local list = {}
    local index = 1
    startPos = startPos + 1
...
local function decode_array(s, startPos)
    if DEBUG then
        print("decode_array", startPos, string_sub(s, startPos))
    end
    local array = {}
    local key, value
    local state = 0
    ...
    while true do
        if DEBUG then
            print("in decode_array", state, key, value)
        end
        local char
        startPos, char = skip_space(s, startPos)
        if char == "}" then
...
decode_any = function(s, startPos, allowStringConstants)
    ...
    if DEBUG then
        print("decode_any", startPos, string_sub(s, startPos))
    end
    local char
    startPos, char = skip_space(s, startPos)
