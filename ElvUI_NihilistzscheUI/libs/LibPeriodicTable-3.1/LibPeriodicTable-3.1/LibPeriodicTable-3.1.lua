...
local PT3, oldminor = LibStub:NewLibrary("LibPeriodicTable-3.1", tonumber(("$Revision: 567 $"):match("(%d+)")) + 90000)
if not PT3 then
    return
end
...
function getItemID(item)
    ...
    return tonumber(item) or tonumber(item:match("item:(%d+)")) or
        (-1 * ((item:match("enchant:(%d+)") or item:match("spell:(%d+)")) or 0))
end
...
function PT3:AddData(arg1, arg2, arg3)
    ...
    assert(
        type(arg2) == "string" or type(arg2) == "table",
        "Invalid arg2: must be set contents string or table, or revision string"
    )
