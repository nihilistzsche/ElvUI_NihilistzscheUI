...
if not _G.LibStub then error(MAJOR_VERSION .. " requires LibStub") end

local NihilistzscheLib = _G.LibStub:NewLibrary(MAJOR_VERSION, MINOR_VERSION)
if not NihilistzscheLib then return end
...
local function NTags_OnEnter(self)
    ...
    if mouseoverSeen[self] then E:UIFrameFadeIn(self, 0.2, 0, 1) end
end

local function NTags_OnLeave(self)
    if not E then E = _G.ElvUI[1] end
    if mouseoverSeen[self] then E:UIFrameFadeOut(self, 0.2, 1, 0) end
end
...
function NihilistzscheLib:UnregisterTag(tag)
    if not self.registeredTags[tag] then return end
...
function NihilistzscheLib:RegisterFontString(key, fs)
    if not self.registeredTagStrings[key] then self.registeredTagStrings[key] = {} end
    self.registeredTagStrings[key].fs = fs
    self.registeredTagStrings[key].backingText = ""
    local frame = fs:GetParent()
    frame:HookScript("OnEnter", function()
        if not NihilistzscheLib.registeredTagStrings[key] then return end
        NTags_OnEnter(fs)
    end)
...
function NihilistzscheLib:UnregisterFontString(key)
    if not self.registeredTagStrings[key] then return end
...
NihilistzscheLib.driverFrame:SetScript("OnEvent", function() NihilistzscheLib:UpdateTagStrings() end)

local CurrentFS, CurrentKey
NihilistzscheLib.tagDirector = {}
setmetatable(NihilistzscheLib.tagDirector, {
    __index = function(_, key)
        local customArgs = key:match("{(.-)}%]")
        if customArgs then key:gsub("{.-}%]", "]") end
...
NihilistzscheLib:RegisterTag("mouseover", function()
