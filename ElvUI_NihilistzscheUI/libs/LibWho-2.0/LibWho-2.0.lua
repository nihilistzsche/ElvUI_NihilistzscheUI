...
function lib.UserInfo(defhandler, name, opts)
	...
	if name:find("%-") and ignoreRealm(name) then
		return
	end
...
function lib:AskWho(args)
	...
	dbg(
		"[" ..
			args.queue ..
				'] added "' .. args.query .. '", queues=' .. #self.Queue[1] .. "/" .. #self.Queue[2] .. "/" .. #self.Queue[3]
	)
	...
	dbg(
		"[" ..
			self.Args.queue ..
				'] returned "' ..
					self.Args.query ..
						'", total=' .. self.Total .. " , queues=" .. #self.Queue[1] .. "/" .. #self.Queue[2] .. "/" .. #self.Queue[3]
	)
	local now = time()
...
function lib:CheckArgument(func, name, argtype, arg, defarg)
	...
	elseif type(arg) == argtype then
		...
		error(
			string.format("%s: '%s' - %s%s expected got %s", func, name, (defarg ~= nil) and "nil or " or "", argtype, type(arg)),
			3
		)
	end -- if
end
...
function lib:CheckPreset(func, name, preset, arg, defarg)
	...
	elseif arg ~= nil and preset[arg] ~= nil then
		...
		error(
			string.format(
				"%s: '%s' - one of %s%s expected got %s",
				func,
				name,
				(defarg ~= nil) and "nil, " or "",
				table.concat(p, ", "),
				self:simple_dump(arg)
			),
			3
		)
	end -- if
...
function lib:CheckCallback(func, prefix, callback, handler, defhandler, nonil)
	...
	elseif type(callback) == "string" then
		...
		error(
			string.format(
				"%s: '%scallback' - %sfunction or string expected got %s",
				func,
				prefix,
				nonil and "nil or " or "",
				type(arg)
