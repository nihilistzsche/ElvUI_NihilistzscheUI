                        ...
                        tier ..
                            "," ..
                                col ..
                                    "," ..
                                        group ..
                                            "," .. tostring(is_inspect) .. "," .. tostring(unit) .. ") returned nil"
        ) --@end-debug@
        return {}
    end
    ...
    if info.class_id then
        ...
        for tier = 1, MAX_TALENT_TIERS do
            for col = 1, NUM_TALENT_COLUMNS do
                local talent, sel =
                    self:GetCachedTalentInfo(info.class_id, tier, col, info.spec_group, is_inspect, unit)
...
function lib:UpdateCommScope()
    local scope =
        ...
        (IsInGroup(LE_PARTY_CATEGORY_HOME) and "PARTY")
...
function lib:CHAT_MSG_ADDON(prefix, datastr, scope, sender)
    ...
    --@debug@
    debug(
        "Incoming LGIST update from " ..
            (scope or "nil") .. "/" .. (sender or "nil") .. ": " .. (datastr:gsub(COMMS_DELIM, ";") or "nil")
    ) --@end-debug@
