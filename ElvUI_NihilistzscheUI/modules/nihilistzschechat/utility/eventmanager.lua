...
function NC.FindURL(event, msg)
    if
        (event == "CHAT_MSG_WHISPER" or event == "CHAT_MSG_BN_WHISPER")
        and CH.db.whisperSound ~= "None"
        and not CH.SoundPlayed
    then
        if msg:sub(1, 3) == "OQ," then return msg end
    ...
    if found > 0 then return CH:GetSmileyReplacementText(CH:CheckKeyword(newMsg)) end
...
function NC:CHAT_MSG_BN_INLINE_TOAST_ALERT(event, toast, author) self:AddStatus(event, toast, author) end
