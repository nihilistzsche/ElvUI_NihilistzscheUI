...
function NC:SetUpHooks()
            ...
            if (editBox:GetAttribute("chatType") == editBox:GetAttribute("stickyType")) then
                if
                    ((editBox:GetAttribute("stickyType") == "PARTY") and (not IsInGroup(LE_PARTY_CATEGORY_HOME)) or
                        ...
                        (editBox:GetAttribute("stickyType") == "INSTANCE_CHAT") and
                    ...
                    if (FCFManager_GetNumDedicatedFrames(chatGroup, chatTarget) == 0) then
                        ...
                        if
                            ((chatType == "WHISPER_INFORM" or chatType == "BN_WHISPER_INFORM") and
                                ...
                                not chatFrame.isNihilistzscheChatFrame
                         then
                            FCFManager_StopFlashOnDedicatedWindows(chatGroup, chatTarget)
                        end
                    end
    ...
    function ChatEdit_ResetChatType(self)
        ...
        if
            ((self:GetAttribute("chatType") == "GUILD" or self:GetAttribute("chatType") == "OFFICER") and
                not IsInGuild())
         then
            self:SetAttribute("chatType", "SAY")
        end
