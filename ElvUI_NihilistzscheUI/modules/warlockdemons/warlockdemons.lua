...
function WD:CreateHeader()
  ...
  local fp, sp =
    self.db.grow == "DOWN" and "TOP" or "BOTTOMLEFT",
    self.db.grow == "DOWN" and "TOPRIGHT" or "BOTTOMRIGHT"
...
function WD:CreateRemoveFunc()
  if not self.RemoveBarByIndex then
    ...
    self.RemoveBarByIndex = function(index)
      tremove(bars, index)
    end
  end
...
function WD:OnSpawn(petGUID)
  ...
  if (petName == "Wild Imp") then
    ...
    label =
      ...
      WILD_IMP_ENERGY_COLORS[petEnergy] and E:RGBToHex(unpack(WILD_IMP_ENERGY_COLORS[petEnergy])) or E:RGBToHex(1, 1, 1),
      petEnergy
    )
