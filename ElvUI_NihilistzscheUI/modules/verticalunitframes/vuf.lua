...
function VUF:UpdateHiddenStatus(frame, event)
  ...
  if (event == "PLAYER_REGEN_DISABLED") then
    frame.disableSeen = true
  elseif (event == "PLAYER_REGEN_ENABLED") then
    frame.disableSeen = nil
  end

  local inCombat = frame.disableSeen
  local isHideOOC = self.db.hideOOC
  local isCasting = frame.isCasting
  local isMouseOver = GetMouseFocus() == frame and frame:IsMouseOver()
  local shouldHide = isHideOOC and not inCombat and not isCasting and not isMouseOver
...
function VUF:Enable()
  ...
  if not elvUFTicker then
    elvUFTicker = C_Timer.NewTicker(2, VUF.UpdateElvUFSetting)
  end
  for _, f in pairs(self.units) do
