---@class NUI
local NUI, E, _, _, P = _G.unpack((select(2, ...)))
...
NI.DatatextSettings = {
    template = {
        ...
        [2] = { point = "TOP", x = 0, numSlots = 1 },
        [3] = { point = "TOP", x = -356, numSlots = 3 },
        [4] = { point = "TOP", x = 356, numSlots = 3 },
        [5] = { point = "TOPRIGHT", x = 0, numSlots = 3 },
        [6] = { point = "BOTTOM", x = 0, numSlots = 1 },
        [7] = { point = "BOTTOM", x = -284, numSlots = 3 },
        [8] = { point = "BOTTOM", x = 284, numSlots = 3 },
    },
    width = {
        [1] = 384,
        [2] = 326,
...
function NI:DatatextPanelSetup()
    ...
    local settings = self.DatatextSettings
    for i = 1, 8 do
        local key = "NUI_DataPanel_" .. i
        db[key] = E:CopyTable({}, E.global.datatexts.newPanelInfo)
        db[key].width = settings.width[i]
        db[key].numPoints = settings.template[i].numSlots
...
function NI:ElvUISetup(role, isSpec)
    ...
    for i = 1, 8 do
        self:SaveMoverPosition(
            ...
            settings.template[i].point,
            E.UIParent,
            settings.template[i].point,
            settings.template[i].x,
            0
        )
    end
    ...
    self:SaveMoverPosition("BelowMinimapContainerMover", "TOPRIGHT", E.UIParent, "TOPRIGHT", -266, -189)
