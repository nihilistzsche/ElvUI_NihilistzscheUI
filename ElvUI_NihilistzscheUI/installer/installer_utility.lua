---@class NUI
local NUI, E, _, _, _, G = _G.unpack((select(2, ...)))
...
function NI:RegisterAddOnInstaller(addonName, func, isElvUIModier, overrideName, characterSpecific)
    ...
    if isElvUIModier then self.ElvUIModifiers[addonName] = func end
    if overrideName then self.InstalledAddOnOverrideNames[string.lower(addonName)] = overrideName end
end
...
function NI.BaseElvUISetup()
    ...
    if not NUIIDB.uiScaleSet or NUI.NihilPrivate then
