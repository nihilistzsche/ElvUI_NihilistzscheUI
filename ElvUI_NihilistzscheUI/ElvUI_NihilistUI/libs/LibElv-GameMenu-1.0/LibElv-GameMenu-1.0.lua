...
local E, L, V, P, G = _G.unpack(ElvUI)
local S = E.Skins
local TT = E.Tooltip
local _G = _G
...
function lib:UpdateHolder()
	...
	for i = 1, total do
		...
		local space
		if (spaceCount[i] and total > spaceCount[i]) then
			space = true
		end
...
_G["GameMenuFrame"]:HookScript(
	...
	function()
		if #lib.buttons <= 5 then
			...
			_G["GameMenuButtonLogout"]:ClearAllPoints()
			_G["GameMenuButtonLogout"]:SetPoint(
				"TOP",
				_G["GameMenuButtonAddons"],
				"BOTTOMLEFT",
				_G["GameMenuFrame"]:GetWidth() / 2 - 25.5,
				-29
			)
		end
	end
...
if not LibStub("LibElvUIPlugin-1.0").plugins[MAJOR] then
	LibStub("LibElvUIPlugin-1.0"):RegisterPlugin(
		MAJOR,
		function()
		end,
		true
