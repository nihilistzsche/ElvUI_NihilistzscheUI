...
COMP.DEV = COMP.IsAddOnEnabled("ElvUIDev")
...
function COMP.Print(addon, feature)
	if (E.private.nihilistui.comp and E.private.nihilistui.comp[addon] and E.private.nihilistui.comp[addon][feature]) then
		return
	end
	print(NUI.Title .. " has |cffff2020disabled|r " .. feature .. " from " .. addon .. " due to incompatiblities.")
	E.private.nihilistui.comp = E.private.nihilistui.comp or {}
	E.private.nihilistui.comp[addon] = E.private.nihilistui.comp[addon] or {}
	E.private.nihilistui.comp[addon][feature] = true
end
...
function COMP:BenikUICompatibility()
	...
	if (Disable(E.db.benikuiDatabars.experience)) then
		changedDatabar = true
	end
	if (Disable(E.db.benikuiDatabars.reputation)) then
		changedDatabar = true
	end
	if (Disable(E.db.benikuiDatabars.azerite)) then
		changedDatabar = true
	end
	if (Disable(E.db.benikuiDatabars.honor)) then
		changedDatabar = true
	end
