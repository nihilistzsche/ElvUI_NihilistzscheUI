...
function ES:ElvUIShadows()
	...
	_G.GameTooltip:HookScript("OnShow", function()
...
function ES:UpdateShadow(shadow)
	local ShadowColor = E.db.nihilistui and E.db.nihilistui.enhancedshadows and E.db.nihilistui.enhancedshadows.shadowcolor
	...
	local size = E.db.nihilistui.enhancedshadows.size
...
function ES:Initialize()
	...
	local framesKeys = {"Arena", "Assist", "Boss", "Party", "Raid", "Raid40", "Raidpet", "Tank"}
	local frameKeys = {
		"Focus",
		"FocusTarget",
		"Pet",
	...
	if (COMP.PA) then
		...
		if EPB.UpdatePetFrame then
			...
			hooksecurefunc(EPB, "InitPetFrameAPI", function()
				self:EPBHook()
			end)
		end
