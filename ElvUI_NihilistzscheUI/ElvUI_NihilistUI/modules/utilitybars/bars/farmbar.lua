...
function FB:CreateBar()
	local bar =
		NUB:CreateBar("NihilistUI_FarmBar", "farmBar", {"TOPLEFT", _G.NihilistUI_TrackerBar, "BOTTOMLEFT", 0, -2}, "Farm Bar")
...
function FB:AddWatch(item, id, target)
	...
	if (not tContains(_G.ElvDB.farmBar[self.myname][table], id)) then
		_G.ElvDB.farmBar[self.myname].count[table][id] = item and GetItemCount(id, true) or select(2, C_CurrencyInfo_GetCurrencyInfo(id))
		tinsert(_G.ElvDB.farmBar[self.myname][table], id)
		if (E.db.nihilistui.utilitybars.farmBar.notify) then
...
function FB:UpdateAndNotify(item, id, count)
	...
	local notify = E.db.nihilistui.utilitybars.farmBar.notify
...
function FB:Initialize()
	...
	local frame = CreateFrame("Frame", "NihilistUI_FarmBarController")
