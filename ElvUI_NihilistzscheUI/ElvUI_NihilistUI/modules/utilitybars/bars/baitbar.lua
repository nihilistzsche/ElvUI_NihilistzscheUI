...
function BB.AddBaits()
	...
	PT:AddData(
		...
		"Rev: 1",
		{
			["NihilistUI.Baits.Draenor"] = "110293, 110291, 110289, 110290, 110292, 110274, 110294, 128229",
			["NihilistUI.Baits.BrokenIsles"] = "133701, 133702, 133703, 133704, 133705, 133706, 133707, 133708, 133709, 133710, 133711, 133712, 133713, 133714, 133715, 133716, 133717, 133719, 133720, 133721, 133722, 133723, 133724, 133795, 139175",
			["NihilistUI.Baits.Shadowlands"] = "173038,173039,173040,173041,173042,173043",
		}
	)
end

function BB:CreateBar()
	local bar = NUB:CreateBar("NihilistUI_BaitBar", "baitBar", {"CENTER", E.UIParent, "CENTER", 0, -280}, "Bait Bar")
...
function BB:UpdateBar(bar)
	...
	if (self.IsInDraenor()) then
		key = "NihilistUI.Baits.Draenor"
		inBaitZone = true
	elseif (self.IsInBrokenIsles()) then
		key = "NihilistUI.Baits.BrokenIsles"
		inBaitZone = true
	elseif (self.IsInShadowlands()) then
		key = "NihilistUI.Baits.Shadowlands"
		inBaitZone = true
	end
...
function BB:Initialize()
	self.AddBaits()
	local frame = CreateFrame("Frame", "NihilistUI_BaitBarController")
