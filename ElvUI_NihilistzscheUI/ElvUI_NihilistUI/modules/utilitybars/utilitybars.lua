...
local nihilistui_ab_id = 770

NUB.CreatedBars = {}

function NUB:PLAYER_REGEN_DISABLED()
	if (E.db.nihilistui.utilitybars.hideincombat) then
...
function NUB:CreateBar(name, db, point, moverName)
	...
	bar.id = nihilistui_ab_id

	if (not AB.db["bar" .. nihilistui_ab_id]) then
		AB.db["bar" .. nihilistui_ab_id] = {showGrid = false}
	end
