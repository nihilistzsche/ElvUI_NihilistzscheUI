...
local function buildCharacterNameString(entry)
	if NUI.Private and not buildClassIconString then
		iconPath = [[Interface\AddOns\ElvUI_NihilistUI_Private\media\textures\]]
...
local function OnEnter(self)
	...
	local tbl = E.global.nihilistui.accountilvl[E.myrealm]
	if (not tbl) then
...
local function addOrUpdateIlvlEntry()
	if (not E.global.nihilistui) then
		E.global.nihilistui = {}
	end

	if (not E.global.nihilistui.accountilvl) then
		E.global.nihilistui.accountilvl = {}
	end

	if (not E.global.nihilistui.accountilvl[E.myrealm]) then
		E.global.nihilistui.accountilvl[E.myrealm] = {}
	end
	...
	local entry = nil
	for _, e in ipairs(E.global.nihilistui.accountilvl[E.myrealm]) do
		if e.name == name then
			entry = e
			break
		end
	...
	if (not entry) then
		...
		tinsert(E.global.nihilistui.accountilvl[E.myrealm], entry)
	else
...
local function OnEvent(self)
	for _, v in pairs(E.global.nihilistui.accountilvl[1] or {}) do
...
local function RmTrackedCharacter(msg)
	...
	local removeIndex = 0
	for i, entry in ipairs(E.global.nihilistui.accountilvl[realm]) do
	...
	if (removeIndex ~= 0) then
		tremove(E.global.nihilistui.accountilvl[realm], removeIndex)
	end
end
...
DT:RegisterDatatext("NihilistUI Account Item Level", 'NihilistUI', {"PLAYER_ENTERING_WORLD", "PLAYER_EQUIPMENT_CHANGED", "PLAYER_AVG_ITEM_LEVEL_UPDATE"}, UpdateDisplay, OnEvent, nil, OnEnter)
