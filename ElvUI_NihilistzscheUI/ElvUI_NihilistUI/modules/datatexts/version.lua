...
local function Click()
	...
	if (ACD) then
		ACD:SelectGroup("ElvUI", "NihilistUI")
	end
end
		...
		NUI.Title ..
			" " ..
				format("v|cff00c0fa%s|r", NUI.Version) ..
					L[" is loaded. For any issues or suggestions, please visit "] ..
						PrintURL("https://git.tukui.org/Nihilistzsche/ElvUI_NihilistUI/issues")
	)
...
local function ValueColorUpdate()
	...
	if (COMP.SLE) then
		if COMP.MERS and COMP.BUI then
			name = name .. ", |cff9482c9S&L|r"
		else
			name = name .. ", |cff9482c9Shadow & Light|r"
		end
		name = name .. format(": |cff99ff33%s|r", _G.ElvUI_SLE[1].version)
	end
	if (COMP.BUI) then
		local BUI = _G.ElvUI_BenikUI[1]
		name = name .. ", |cff00c0faBenikUI|r" .. format(": |cff99ff33%s|r", BUI.Version)
	end
	if (COMP.MERS) then
		local MER = _G.ElvUI_MerathilisUI[1]
		if (COMP.SLE and COMP.BUI) then
			name = name .. ", |cffff7d0aMeraUI|r"
		else
			name = name .. ", |cffff7d0aMerathilisUI|r"
		end
		name = name .. format(": |cff99ff33%s|r", MER.Version)
	end
...
DT:RegisterDatatext("NihilistUI Version", 'NihilistUI', {"PLAYER_ENTERING_WORLD"}, OnEvent, nil, Click, OnEnter)
