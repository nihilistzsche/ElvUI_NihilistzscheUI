...
function RCD:EvaluateModifier(unitInfo, modifierTbl, modifierType)
	if (type(modifierTbl) ~= "table") then
		return 0
	end
...
function RCD.EvaluatePlayerTalent(unitInfo, modifierTbl)
	if (not unitInfo.talents) then
		return false
	end
	for _, tbl in pairs(unitInfo.talents) do
		if (select(6, GetTalentInfoByID(tbl.talent_id)) == modifierTbl.id) then
			return true
		end
	end
	return false
...
function RCD.EvaluatePlayerPvPTalent(unitInfo, modifierTbl)
	if (not unitInfo.pvpTalents) then
		return false
	end
	if (tContains(unitInfo.pvpTalents, modifierTbl.id)) then
		return true
	end
	return false
end
...
function RCD.EvaluatePlayerLegendary(unitInfo, modifierTbl)
	if
		(not RCD.cached_players[unitInfo.guid] or not RCD.cached_players[unitInfo.guid].items or
			not RCD.cached_players[unitInfo.guid].level)
	 then
		return false
	end
	if (RCD.cached_players[unitInfo.guid].level > 115) then
		return false
	end
	if (RCD.cached_players[unitInfo.guid].items[modifierTbl.slot] == modifierTbl.id) then
		return true
	end
	return false
end
