...
function VUF:IsDefault(settingstring)
	...
	local options, profile = self.db, P.nihilistui.vuf
	for _, setting in pairs(settings) do
		options = options[setting]
		profile = profile[setting]
		if not profile then -- this only happens for customTexts
			profile = {}
			E:CopyTable(profile, P.nihilistui.vuf.customtext)
		end
	end
...
function VUF.GetUnitFrame(unit)
	...
	return "NihilistUF_" .. stringTitle
end
...
function VUF:AddCustomText(unit, name, optionsTbl)
	...
	if not E.db.nihilistui.vuf.units[unit][name] then
		E.db.nihilistui.vuf.units[unit][name] = {}
		E:CopyTable(E.db.nihilistui.vuf.units[unit][name], P.nihilistui.vuf.customtext)
		if not E.db.nihilistui.vuf.units[unit].customTexts then
			E.db.nihilistui.vuf.units[unit].customTexts = {}
		end
		E.db.nihilistui.vuf.units[unit].customTexts[name] = true
	end
	optionsTbl = optionsTbl or E.Options.args.NihilistUI.args.modules.args.VerticalUnitFrames;
	if optionsTbl.args[unit] then
		optionsTbl.args[unit].args[name] =
			self:GenerateElementOptionsTable(unit, name, 4000, E:StringTitle(name), true, false, false, true, false, false)
		optionsTbl.args[unit].args[name].args.delete = {
			type = "execute",
			order = 1,
			name = DELETE,
			func = function()
				E.db.nihilistui.vuf.units[unit].customTexts[name] = nil
				E.db.nihilistui.vuf.units[unit][name] = nil
...
function VUF:SetUpCustomTexts(frame, optionsTbl)
	...
	if not E.db.nihilistui.vuf.units[unit] then
		return
	end
	if E.db.nihilistui.vuf.units[unit].customTexts then
		for textName, _ in pairs(E.db.nihilistui.vuf.units[unit].customTexts) do
...
function VUF:ConstructVerticalUnitFrame(frame, unit)
	...
	frame:SetScript(
		...
		function(_self)
			...
			if
				E.db.nihilistui.vuf.hideOOC and not UnitAffectingCombat("player") and not UnitAffectingCombat("pet") and
					_self.unit ~= "target"
			 then
...
function VUF:UpdateFrame(unit)
	...
	if E.db.nihilistui.vuf.enabled and self.db.units[unit].enabled then
...
function VUF:HookSetAlpha(frame)
	if (frame._NihilistUI_SetAlpha) then
		return
	end
	frame._NihilistUI_SetAlpha = frame.SetAlpha
	frame.SetAlpha = function(_self, alpha)
		if not alpha then return end
		if (InCombatLockdown()) then
			_self:_NihilistUI_SetAlpha(self.db.alpha)
		else
			_self:_NihilistUI_SetAlpha(alpha)
		end
	end
end
...
function VUF:ResetUnitSettings(unit)
	...
	E:CopyTable(self.db.units[unit], P.nihilistui.vuf.units[unit])
