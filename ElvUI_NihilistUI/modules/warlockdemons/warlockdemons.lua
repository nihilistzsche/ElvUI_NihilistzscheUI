...
function WD:CreateHeader()
	...
	local container = CreateFrame("Frame", "NihilistUIWarlockDemonsContainer", header, "BackdropTemplate")
	container:SetFrameLevel(header:GetFrameLevel())
	local fp, sp = self.db.grow == "DOWN" and "TOP" or "BOTTOMLEFT", self.db.grow == "DOWN" and "TOPRIGHT" or "BOTTOMRIGHT"
	...
	E:CreateMover(header, "WarlockDemonsMover", L["Demon Tracker"], nil, nil, nil, "ALL,SOLO,NIHILISTUI")

	return header
end

function WD:ShouldAttachToNamePlate()
	return NP.UpdatePlateGUID and self.db.attachToNamePlate and tonumber(GetCVar("nameplateShowFriendlyMinions")) == 1 and
		not self.nameplatesForbidden
end
...
function WD:CreateBar(icon, duration, guid)
	...
	if self:ShouldAttachToNamePlate() then
		...
		if NP.PlateGUID[guid] then
			self.AttachBarToNamePlate(bar, guid)
		end -- Otherwise well attach when we find one.
	else
...
function WD:RemoveBar(bar)
	local toRemove =
		type(bar) == "number" and bar or
		FindInTableIf(
			self.activeBars,
			function(v)
				return v.petGUID == bar.petGUID
			end
		)
...
function WD:UpdateBars(isDemonicTyrant)
	...
	if (#remove_queue > 0 or #add_queue > 0) then
		self:UpdateBars()
	end
end
...
function WD:UpdateNameplateForbiddenFlag(flag)
	self.nameplatesForbidden = flag
end
...
function WD:InitializeNPHooks()
	...
	hooksecurefunc(
		...
		function()
			NP.StyleFilterTriggerEvents.FAKE_WDForceUpdate = 0
		end
	)
	NP:StyleFilterConfigure()
	self.styleFilterUpdateLoop = C_Timer_NewTicker(1, self.UpdateQueuedUpdateNPs)
	NP:StyleFilterAddCustomCheck("NihilistUI_WarlockDemons", self.StyleFilterCustomCheck)
...
local function CreateWDRespawnClosure(petGUID)
	if closures[petGUID] then
		return closures[petGUID]
	end
...
function WD:OnSpawn(petGUID)
	...
	if (strmatch(petName, UnitName("pet") or "ThisStringShouldNeverComeBackAsAPetNameSoShouldNeverMatch")) then
		return
	end
...
function WD.StyleFilterCustomCheck(frame, _, trigger)
	local passed = nil
	if (_G.UnitIsUnit(frame.unit, "pet")) then
		return false
	end
	if (trigger.isDemonologyWarlockDemonNUI or trigger.isNotDemonologyWarlockDemonNUI) then
		...
		if (guid) then
			...
			if
				(trigger.isDemonologyWarlockDemonNUI and isDemonologyWarlockDemonNUI or
					trigger.isNotDemonologyWarlockDemonNUI and not isDemonologyWarlockDemonNUI)
			 then
				passed = true
			else
				return false
...
function WD:Initialize()
	...
	_G.zPets.RegisterPetEvent(
		"OnDespawn",
		function(petGUID)
			WD:OnDespawn(petGUID)
		end
	)

	GetPetName = _G.zPets.GetPetName

	if NP.UpdatePlateGUID then
