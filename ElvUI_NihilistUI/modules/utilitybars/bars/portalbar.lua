...
function PRB.CreateBar()
	local bar = NUB:CreateBar(
		"NihilistUI_PortalBar",
		"portalBar",
		{"BOTTOMRIGHT", _G.NihilistUI_ProfessionBar, "TOPRIGHT", 0, 2},
		"Portal Bar"
	)
	if COMP.SLE then
...
function PRB.UpdateBarKeys(bar)
	bar.keys = {
		"NihilistUI.TeleportSpells.Teleports",
		"NihilistUI.TeleportSpells.Portals",
		{"Misc.Hearth", "NihilistUI.Misc.Hearth"}
	}

	if (bar.db.challengeModePandaria) then
		tinsert(bar.keys, 3, "NihilistUI.TeleportSpells.ChallengeMode.Pandaria")
	end

	if (bar.db.challengeModeDraenor) then
		tinsert(bar.keys, 4, "NihilistUI.TeleportSpells.ChallengeMode.Draenor")
	end
end
...
function PRB.AddNihilsitUIData()
	-- luacheck: no max line length
	PT:AddData(
		"NihilistUI.TeleportSpells",
		"Rev: 1",
		{
			["NihilistUI.TeleportSpells.Teleports"] = "-556,-53140,-120145,-3565,-32271,-3562,-3567,-33690,-35715,-32272,-49358,-176248,-3561,-49359,-3566,-88342,-88344,-3563,-132621,-132627,-176242,-18960,-50977,-126892,-193753,-193759, -224768, -281403, -281404, -312372,-344587",
			["NihilistUI.TeleportSpells.Portals"] = "-53142, -120146, -11419, -32266, -11416, -11417, -33691, -35717, -32267, -49361, -176246, -10059, -49360, -11420, -88345, -88346, -11418, -132620, -132626, -176244, -224871, -281400, -281402,-344597",
			["NihilistUI.TeleportSpells.ChallengeMode.Pandaria"] = "-131228, -131204, -131222, -131225, -131206, -131205, -131229, -131231, -131232",
			["NihilistUI.TeleportSpells.ChallengeMode.Draenor"] = "-169764, -169762, -169766, -169763, -169769, -169765, -169767, -169769"
		}
	)
	local nihilistHearth = "163073,180817"
	if E.myrace == "GNOME" then
		nihilistHearth = nihilistHearth .. ", 168862"
	end
	PT:AddData(
		"NihilistUI.Misc",
		"Rev: 1",
		{
			["NihilistUI.Misc.Hearth"] = nihilistHearth
		}
	)
end
...
function PRB:Initialize()
	self.AddNihilsitUIData()
	NUB:InjectScripts(self)

	local frame = CreateFrame("Frame", "NihilistUI_PortalBarController")
