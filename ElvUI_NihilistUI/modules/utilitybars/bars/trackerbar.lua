...
function TB:CreateBar()
	local bar =
		NUB:CreateBar("NihilistUI_TrackerBar", "trackerbar", {"TOPLEFT", E.UIParent, "TOPLEFT", 0, -20}, "Tracker Bar")
...
function TB:AddWatch(item, id)
	...
	if (not tContains(ElvDB.trackerbar[TB.myname][table], id)) then
		ElvDB.trackerbar[TB.myname].count[table][id] = item and GetItemCount(id, true) or (C_CurrencyInfo_GetCurrencyInfo(id)).quantity
		tinsert(ElvDB.trackerbar[TB.myname][table], id)
		if (E.db.nihilistui.utilitybars.trackerbar.notify) then
			local string = item and notificationItem or notificationCurrency
			UIErrorsFrame:AddMessage(string:format(item and select(2, GetItemInfo(id)) or C_CurrencyInfo_GetCurrencyLink(id)))
		end
	end
...
function TB:UpdateAndNotify(item, id, count)
	...
	local notify = E.db.nihilistui.utilitybars.trackerbar.notify
...
function TB:Initialize()
	...
	--[[if (not E.global.nihilistui.farmmigrated) then
		NUI:MigrateFarmDataToTracker();
		E.global.nihilistui.farmmigrated = true;
	end]]
	TB.sessionDB = {}
	TB.sessionDB.items = {}
	...
	local frame = CreateFrame("Frame", "NihilistUI_TrackerBarController")
