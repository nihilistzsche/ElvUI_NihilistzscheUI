...
function NC:SetUpHooks()
	...
	local function CheckDeactivate(editBox, ignoreLast)
		if (ACTIVE_CHAT_EDIT_BOX and ACTIVE_CHAT_EDIT_BOX ~= editBox) then
			if
				not ignoreLast and self.activeChatEditBox ~= LAST_ACTIVE_CHAT_EDIT_BOX and
					LAST_ACTIVE_CHAT_EDIT_BOX ~= ACTIVE_CHAT_EDIT_BOX
			 then
	...
	function ChatEdit_ActivateChat(editBox, ignoreLast)
		...
		if not editBox.isNihilistChatFrame then
	...
	function ChatFrame_OpenChat(text, chatFrame)
		...
		if not editBox.isNihilistChatFrame then
			if (editBox:GetAttribute("chatType") == editBox:GetAttribute("stickyType")) then
				if
					((editBox:GetAttribute("stickyType") == "PARTY") and (not IsInGroup(LE_PARTY_CATEGORY_HOME)) or
						(editBox:GetAttribute("stickyType") == "RAID") and (not IsInRaid(LE_PARTY_CATEGORY_HOME)) or
						(editBox:GetAttribute("stickyType") == "INSTANCE_CHAT") and (not IsInGroup(LE_PARTY_CATEGORY_INSTANCE)))
				 then
	...
	hooksecurefunc(
		...
		function(self, event, ...)
			...
			if (strsub(event, 1, 9) == "CHAT_MSG_") then
				...
				if
					((chatGroup == "BN_WHISPER" or chatGroup == "WHISPER") and
						(GetCVar("whisperMode") == "popout" or GetCVar("whisperMode") == "popout_and_inline"))
				 then
					...
					if (FCFManager_GetNumDedicatedFrames(chatGroup, chatTarget) == 0) then
						...
						if
							((event == "CHAT_MSG_WHISPER_INFORM" or event == "CHAT_MSG_BN_WHISPER_INFORM") and
								GetCVar("whisperMode") == "popout")
						 then
							if not chatFrame.isNihilistChatFrame then
						...
						if
							((chatType == "WHISPER_INFORM" or chatType == "BN_WHISPER_INFORM") and
								GetCVar("whisperMode") == "popout_and_inline") or
								not chatFrame.isNihilistChatFrame
						 then
							FCFManager_StopFlashOnDedicatedWindows(chatGroup, chatTarget)
						end
					end
	...
	hooksecurefunc(
		"FCF_DockFrame",
		function(frame, index, selected)
			CURRENT_CHATFRAME = frame
		end
	)
	...
	hooksecurefunc(
		...
		function(chatType, chatTarget, sourceChatFrame, selectWindow)
	...
	function ChatEdit_ResetChatType(self)
		...
		if not self.isNihilistChatFrame then
	...
	function ChatEdit_SetDeactivated(editBox)
		...
		if (editBox:GetName()) then
			if _G[editBox:GetName() .. "Language"] then
