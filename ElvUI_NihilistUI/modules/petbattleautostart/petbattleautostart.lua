...
function PBAS:AutoTrainerStart(event)
	if event == "GOSSIP_SHOW" or event == "QUEST_DETAIL" or event == "QUEST_PROGRESS" or event == "QUEST_COMPLETE" then
		...
		if UnitExists("npc") then -- (Fixes edge case where the tamer has been detargeted)
			...
			if tamerID and self.PetTamers[tamerID] and self.PetTamers[tamerID].valid then
				validTamer = true
			end
		...
		if validTamer then
			if event == "GOSSIP_SHOW" then
				...
				elseif self.db.autoQuestAccept and C_GossipInfo.GetNumActiveQuests() > 0 and questindex then
					...
					C_GossipInfo.CloseGossip()
				elseif
					self.db.autoStartBattle and C_GossipInfo.GetNumAvailableQuests() == 0 and C_GossipInfo.GetNumOptions() > 0 and
						not self.PetTamers[tamerID].noBattle
				 then
					if HasControl() and not InCombat() and not InPetBattle() then
						...
						for i = #gossipOptions, 1, -1 do
							if gossipOptions[i].type ~= "gossip" then
								battleOption = i
								break
							end
						end
						...
						if battleOption or #gossipOptions == 1 then
...
function PBAS:Initialize()
	self.db = E.db.nihilistui.petbattleautostart
	local TrainerEvents = {
		"GOSSIP_SHOW",
		"QUEST_DETAIL",
		"QUEST_PROGRESS",
