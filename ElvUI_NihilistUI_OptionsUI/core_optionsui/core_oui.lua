...
local CCFG = NUI:NewModule("NihilistUIConfig")
local EP = E.Libs.EP
...
function CCFG.GenerateOptions()
	...
	local function CreateButton(number, text, ...)
		...
		local options = {
			...
			name = text,
			func = function()
				ACD:SelectGroup("ElvUI", "NihilistUI", unpack(path))
			end
		}
		return options
	end
	--Main options group
	E.Options.args.NihilistUI = {
		order = 77,
		type = "group",
		name = [[|TInterface\AddOns\ElvUI_NihilistUI\media\textures\nihilistui_logo:12:12|t|cffff2020NihilistUI|r]],
		args = {
			header = {
				order = 1,
				type = "header",
				name = "|cfff02020NihilistUI|r|r" .. format(": |cff99ff33%s|r", NUI.Version)
			},
			logo = {
				type = "description",
				name = [=[|cffff2020NihilistUI|r is an extension for ElvUI. It adds:
- a lot of new and exclusive features.
- more customization options for existing ones.]=],
				order = 2,
				image = function()
					return "Interface\\AddOns\\ElvUI_NihilistUI\\media\\textures\\elvui_nihilistui_logo", 200, 100
				end
			},
			sep1 = {
				type = "description",
			...
			InstallerFont = {
				...
				get = function()
					return E.global.nihilistui.installer.font
				end,
				set = function(_, value)
					E.global.nihilistui.installer.font = value
				end
			},
			InstallerTexture = {
				order = 6,
				...
				get = function()
					return E.global.nihilistui.installer.texture
				end,
				set = function(_, value)
					E.global.nihilistui.installer.texture = value
				end
			},
			installerFontSize = {
					type = "range",
	...
	for _, mod in NUI.PairsByKeys(mods) do
		...
		if (options) then
			...
			E.Options.args.NihilistUI.args.modules.args[_name] = options
			E.Options.args.NihilistUI.args.modules.args[_name].order = order
			E.Options.args.NihilistUI.args.buttons.args[_name .. "Button"] =
				CreateButton(order, E.Options.args.NihilistUI.args.modules.args[_name].name, "modules", _name)
			order = order + 1
		end
	end
end
...
function CCFG:Initialize()
	EP:RegisterPlugin("ElvUI_NihilistUI", self.GenerateOptions)
end
