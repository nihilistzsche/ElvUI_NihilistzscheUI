...
function RCD:GenerateOptions()
	local options = {
		...
		args = {
			...
			description = {
				order = 2,
				type = "description",
				name = L["NihilistUI Raid CDs provides a list of raid cds separated by category.\n"]
			},
			general = {
				order = 3,
				type = "group",
				...
				args = {
					...
					resetsettings = {
						...
						func = function()
							E:CopyTable(E.db.nihilistui.raidcds, P.nihilistui.raidcds)
							self:UpdateEnableState()
							self:UpdateMedia()
	...
	local function AddOptionsForCategory(category, order)
		local categoryOptions = {
			...
			type = "group",
			get = function(info)
				return self.db.cooldowns[category][tonumber(info[#info])]
			end,
			set = function(info, value)
