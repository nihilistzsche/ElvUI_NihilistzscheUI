...
function DBN.GenerateOptions()
	local options = {
		...
		get = function(info)
			return E.db.nihilistui.databarnotifier[info[#info]]
		end,
		set = function(info, value)
			E.db.nihilistui.databarnotifier[info[#info]] = value
		end,
		args = {
			header = {
				order = 1,
				type = "header",
				name = L["NihilistUI DataBarNotifier by Nihilistzsche"]
			},
			description = {
				order = 2,
				type = "description",
				name = L[
					"NihilistUI DataBarNotifier prints out messages to a given chat frame when you gain experience, reputation, artifact xp, or honor.\n"
				]
			},
			general = {
				order = 3,
				...
				args = {
					...
					resetsettings = {
						...
						func = function()
							E:CopyTable(E.db.nihilistui.databarnotifier, P.nihilistui.databarnotifier)
						end
					}
				}
			},
