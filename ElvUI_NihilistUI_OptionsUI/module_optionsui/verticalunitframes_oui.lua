local NUI, E, L, _, P, _ = _G.unpack(_G.ElvUI_NihilistUI) --Inport: Engine, Locales, ProfileDB, GlobalDB
...
function VUF:GenerateElementOptionsTable(
	...
	local options = {
		...
		name = L[name],
		get = function(info)
			return E.db.nihilistui.vuf.units[unit][element][info[#info]]
		end,
		set = function(info, value)
			E.db.nihilistui.vuf.units[unit][element][info[#info]] = value
			VUF:UpdateAllFrames()
		end,
		args = {
			enabled = {
	...
	if hasAnchor then
		options.args.anchor = {
			...
			guiInline = true,
			get = function(info)
				return E.db.nihilistui.vuf.units[unit][element].anchor[info[#info]]
			end,
			set = function(info, value)
				E.db.nihilistui.vuf.units[unit][element].anchor[info[#info]] = value
				VUF:UpdateAllFrames()
			end,
			args = {
				pointTo = {
		...
		if not ((unit == "player" or unit == "target") and element == "castbar") then
			if element ~= "aurabars" then
				options.args.size = {
					...
					guiInline = true,
					get = function(info)
						return E.db.nihilistui.vuf.units[unit][element].size[info[#info]]
					end,
					set = function(info, value)
						E.db.nihilistui.vuf.units[unit][element].size[info[#info]] = value
						VUF:UpdateAllFrames()
					end,
					args = {
						width = {
			...
			options.args.size = {
				...
				args = {
					horizontal = {
						...
						guiInline = true,
						get = function(info)
							return E.db.nihilistui.vuf.units[unit][element].size.horizontal[info[#info]]
						end,
						set = function(info, value)
							E.db.nihilistui.vuf.units[unit][element].size.horizontal[info[#info]] = value
							VUF:UpdateAllFrames()
						end,
						args = {
							width = {
					...
					vertical = {
						...
						guiInline = true,
						get = function(info)
							return E.db.nihilistui.vuf.units[unit][element].size.vertical[info[#info]]
						end,
						set = function(info, value)
							E.db.nihilistui.vuf.units[unit][element].size.vertical[info[#info]] = value
							VUF:UpdateAllFrames()
						end,
						args = {
							width = {
	...
	if hasValue then
		options.args.value = {
			...
			guiInline = true,
			get = function(info)
				return E.db.nihilistui.vuf.units[unit][element].value[info[#info]]
			end,
			set = function(info, value)
				E.db.nihilistui.vuf.units[unit][element].value[info[#info]] = value
				VUF:UpdateAllFrames()
			end,
			args = {
				enabled = {
				...
				anchor = {
					...
					guiInline = true,
					get = function(info)
						return E.db.nihilistui.vuf.units[unit][element].value.anchor[info[#info]]
					end,
					set = function(info, value)
						E.db.nihilistui.vuf.units[unit][element].value.anchor[info[#info]] = value
						VUF:UpdateAllFrames()
					end,
					args = {
						attachTo = {
		...
		options.args.spacing = {
			...
			args = {
				...
				spacesettings = {
					...
					guiInline = true,
					get = function(info)
						return E.db.nihilistui.vuf.units[unit][element].spacesettings[info[#info]]
					end,
					set = function(info, value)
						E.db.nihilistui.vuf.units[unit][element].spacesettings[info[#info]] = value
						VUF:UpdateAllFrames()
					end,
					args = {
						offset = {
	...
	if element == "castbar" then
		...
		if hasTicks then
			...
			options.args.tickcolor = {
				...
				name = L["Tick Color"],
				get = function(info)
					local t = E.db.nihilistui.vuf.units[unit][element][info[#info]]
	...
	if element == "cutaway" then
		options.args.health = {
			...
			name = L.Health,
			get = function(info)
				return E.db.nihilistui.vuf.units[unit].cutaway.health[info[#info]]
			end,
			set = function(info, value)
				E.db.nihilistui.vuf.units[unit].cutaway.health[info[#info]] = value
				VUF:UpdateAllFrames()
			end,
			args = {
				enabled = {
				...
				lengthBeforeFade = {
					...
					step = 0.1,
					disabled = function()
						return not E.db.nihilistui.vuf.units[unit].cutaway.health.enabled
					end
				},
				fadeOutTime = {
					type = "range",
		...
		options.args.power = {
			...
			guiInline = true,
			get = function(info)
				return E.db.nihilistui.vuf.units[unit].cutaway.power[info[#info]]
			end,
			set = function(info, value)
				E.db.nihilistui.vuf.units[unit].cutaway.power[info[#info]] = value
				VUF:UpdateAllFrames()
			end,
			args = {
				enabled = {
...
function VUF:GenerateUnitOptionTable(unit, name, order, mover, elements)
	local options = {
		...
		childGroups = "select",
		get = function(info)
			return E.db.nihilistui.vuf.units[unit][info[#info]]
		end,
		set = function(info, value)
			E.db.nihilistui.vuf.units[unit][info[#info]] = value
			VUF:UpdateAllFrames()
		end,
		args = {
			enabled = {
			...
			customText = {
				...
				set = function(_, textName)
					for object, _ in pairs(E.db.nihilistui.vuf.units[unit]) do
...
function VUF:GenerateOptionTables(options)
	...
	for _, unit in ipairs(optionUnits) do
		...
		options.args[unit] = self:GenerateUnitOptionTable(unit, optionName, order, optionName, self.units[unit].elements)
...
function VUF:GenerateOptions()
	...
	-- luacheck: push no max line length
	E.Options.args.unitframe.args.generalOptionsGroup.args.generalGroup.args.fontGroup.args.font.set = function(info, value)
		E.db.unitframe[info[#info]] = value
	...
	local options = {
		...
		args = {
			header = {
				order = 1,
				type = "header",
				name = L["NihilistUI VerticalUnitFrames by Nihilistzsche"]
			},
			description = {
				order = 2,
				type = "description",
				name = L[
					"NihilistUI VerticalUnitFrames provides a configurable centered, vertical unit frame option for use with ElvUI.\n"
				]
			},
			credits = {
				order = 10000,
				...
				args = {
					...
					credits = {
						order = 2,
						type = "description",
						name = L.NihilistUI_VerticalUnitFrames_CREDITS
					}
				}
			}
		}
	...
	options.args.vufgen = {
		...
		args = {
			enabled = {
				...
				get = function(info)
					return E.db.nihilistui.vuf[info[#info]]
				end,
				set = function(info, value)
					E.db.nihilistui.vuf[info[#info]] = value
					self:Enable()
				end
			},
			resetsettings = {
				...
				func = function()
					E:CopyTable(E.db.nihilistui.vuf, P.nihilistui.vuf)
					self:Enable()
	...
	options.args.vufOptions = {
		...
		args = {
			...
			enableMouse = {
				...
				disabled = function()
					return E.db.nihilistui.vuf.hideElv
				end
			},
			alpha = {
				type = "range",
