...
local NI = E.Libs.AceAddon:NewAddon("NinihilistUI_Installer")
NUI.Installer = NI
...
function NI:InstallForClass(class)
	local role =
		type(self.ClassSpecProfiles[class]) == "table" and self.ClassSpecProfiles[class][1] or self.ClassSpecProfiles[class]
...
NI.installTable = {
	...
	tutorialImage = [[Interface\AddOns\ElvUI_NihilistUI\media\textures\elvui_nihilistui_logo.tga]],
	Pages = {
		[1] = function()
		...
		[2] = function()
			...
			PluginInstallFrame.Desc1:SetText(
				"Choose your class if you would like to setup NihilistUI to match the authors defaults for your role.\nChoose finish if you would like to keep your current setup."
			)
			PluginInstallFrame.Desc2:SetText("")
...
function NI:Initialize()
	...
	self.db = E.global.nihilistui.installer
