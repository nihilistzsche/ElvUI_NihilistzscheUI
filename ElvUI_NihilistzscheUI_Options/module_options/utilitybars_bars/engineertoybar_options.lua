...
function ETOYB:GenerateUtilityBarOptions()
    if not self:IsEngineer() then return end
    local options = {
        type = "group",
        ...
        args = {
            header = {
                order = 1,
                type = "header",
                name = L["NihilistzscheUI EngineerToyBar by Nihilistzsche"],
            },
            description = {
                order = 2,
                type = "description",
                name = L["NihilistzscheUI EngineerToyBar provides an automatically updated bar populated with your engineering toys."],
            },
            general = {
                order = 3,
                type = "group",
                name = L.General,
                guiInline = true,
                get = function(info) return E.db.nihilistzscheui.utilitybars.engineertoybar[info[#info]] end,
                set = function(info, value)
                    E.db.nihilistzscheui.utilitybars.engineertoybar[info[#info]] = value
                ...
                args = {
                    ...
                    resetsettings = {
                        ...
                        func = function()
                            E:CopyTable(
                                E.db.nihilistzscheui.utilitybars.engineertoybar,
                                P.nihilistzscheui.utilitybars.engineertoybar
                            )
                            self:UpdateBar(self.bar)
                        end,
                    },
            ...
            itemToggles = {
                ...
                get = function(info)
                    ...
                    local val = E.db.nihilistzscheui.utilitybars.engineertoybar.toys[toyID]
                    if val ~= nil then
                        return val
                    else
                        return true
                ...
                set = function(info, value)
                    local toyID = tonumber(info[#info])
                    E.db.nihilistzscheui.utilitybars.engineertoybar.toys[toyID] = value
                    self:UpdateBar(self.bar)
                end,
                args = {},
            },
