...
function NUB:GenerateOptions()
    local options = {
        type = "group",
        name = "Utility Bars",
        desc = "Various Utility Bars provided by NihilistzscheUI",
        childGroups = "select",
        args = {
            hideincombat = {
                type = "toggle",
                ...
                get = function(info) return E.db.nihilistzscheui.utilitybars[info[#info]] end,
                set = function(info, value) E.db.nihilistzscheui.utilitybars[info[#info]] = value end,
            },
        },
    }
    ...
    for _, mod in pairs(self.RegisteredBars) do
        if mod.GenerateUtilityBarOptions then bars[mod:GetName()] = mod end
    end
