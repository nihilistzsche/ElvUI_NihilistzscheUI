...
function NI:ElvUIFCTSetup()
	...
	local unitFrames = {"Player", "Target", "TargetTarget", "TargetTargetTarget", "Focus", "FocusTarget", "Pet", "PetTarget"}
	local udb = {
		enable = true,
		showName = true,
		showIcon = true,
	...
	for _, frame in ipairs(groupUnitFrames) do
		...
		if frame == "Party" or frame == "Raid" or frame == "Raid40" or frame == "RaidPet" or frame == "Assist" or frame == "Tank" then
			db.unitframes.frames[frame].showHots = true
		else
			db.unitframes.frames[frame].showDots = true
		end
