...
function NI:ElvUINonHealerSetup()
	...
	self.SaveMoverPosition("ElvUF_RaidMover", "BOTTOMLEFT", "DTPanelDTB2_NihilistUITMover", "TOPLEFT", 0, 4)
	self.SaveMoverPosition("ElvUF_RaidpetMover", "BOTTOMLEFT", "ElvUF_RaidMover", "BOTTOMRIGHT", 4, 0)
	self.SaveMoverPosition("ElvUF_Raid40Mover", "BOTTOMLEFT", "DTPanelDTB2_NihilistUITMover", "TOPLEFT", 0, 4)
	self.SaveMoverPosition("ElvUF_PartyMover", "BOTTOMLEFT", "DTPanelDTB2_NihilistUITMover", "TOPLEFT", 0, 4)
...
function NI:GlobalNameplateSetup()
	...
	wipe(E.global.nameplate.filters or {})
	local classes = {}
	...
	for c, filterClassName in pairs(classes) do
		...
		if needsPetFilter then
			E.global.nameplate.filters["My_Pet_" .. filterClassName] = {
				actions = {
					color = {
		...
		if self.currentClass == "WARLOCK" then
			E.global.nameplate.filters.Demonology_Warlock_Demon_AboutToExpire = {
				actions = {
					flash = {
						enable = true,
						speed = 6
		...
		E.global.nameplate.filters["My_Minions_" .. filterClassName] = {
			actions = {
				tags = {
					title = " "
				},
	...
	if not NUI.Lulupeep then
		E.global.nameplate.filters.Player_NameHealth_NonTarget = {
			actions = {
				nameOnly = true
			},
			triggers = {
		...
		E.global.nameplate.filters.Friendly_NameHealth_NonTarget = {
			actions = {
				nameOnly = true
			},
			triggers = {
	...
	if COMP.CT and COMP.IsAddOnEnabled("TotalRP3") and COMP.IsAddOnEnabled("RP_Tags") then
		E.global.nameplate.filters.PlayerHasRPProfile = {
			actions = {
				tags = {
...
function NI.NihilistDatatextPanelSetup()
	...
	copy("NihilistUIR")
	copy("NihilistUIT")
	db.DTB2_NihilistUIT.numPoints = 1
	if NUI.Private then
		copy("NihilistUILT")
		db.DTB2_NihilistUILT.numPoints = 1
	end
end
...
function NI:ElvUISetup(role, isSpec)
	...
	self:EDB().datatexts.panels.DTB2_NihilistUI = {
		enable = true,
		"DungeonTools",
		"QuickJoin",
		"Improved System"
	}
	self:EDB().datatexts.panels.DTB2_NihilistUIR = {
		enable = true,
		"FamilyFamiliarHelper",
		"Rematch",
		"tdBattlePetScript"
	}
	self:EDB().datatexts.panels.DTB2_NihilistUIT = {
		enable = true,
		"Rarity"
	}
	local NUI_DataPanel_6_LeftDT = "Quick Join"
	if (NUI.Private) then
		NUI_DataPanel_6_LeftDT = "NihilistUI Dungeon/Raid Difficulty"
	end
	self:EDB().datatexts.panels.NUI_DataPanel_1 = {
		enable = true,
		"NihilistUI Account Item Level",
		"War Mode",
	...
	if NUI.Private then
		self:EDB().datatexts.panels.DTB2_NihilistUILT = {
			enable = true,
			[1] = "NihilistUI Pet Challenge Tracker"
		}
	end
	...
	self.SaveMoverPosition("DTPanelDTB2_NihilistUIMover", "BOTTOM", _G.LeftChatPanel, "TOP", 0, 4)
	self.SaveMoverPosition("DTPanelDTB2_NihilistUIRMover", "BOTTOM", _G.RightChatPanel, "TOP", 0, 4)
	self.SaveMoverPosition("DTPanelDTB2_NihilistUITMover", "BOTTOM", "DTPanelDTB2_NihilistUIMover", "TOP", 0, 4)
	if NUI.Private then
		self.SaveMoverPosition("DTPanelDTB2_NihilistUILTMover", "BOTTOM", "DTPanelDTB2_NihilistUIRMover", "TOP", 0, 4)
	end
	for i = 1, 8 do
...
function NI:ElvUIGlobalSetup()
	...
	if not NUI.Lulupeep then
		self:NihilistDatatextPanelSetup()
	end
end
