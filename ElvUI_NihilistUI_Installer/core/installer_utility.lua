local NUI, E, _, _, _, G = _G.unpack(_G.ElvUI_NihilistUI)
...
function NI.SaveMoverPosition(mover, anchor, parent, point, x, y)
	...
	NI:EDB().movers[mover] = format("%s,%s,%s,%d,%d", anchor, type(parent) == "string" and parent or parent:GetName(), point, x, y)
end
...
function NI:InitBaseProfile(n, r)
	...
	if not n then
		self.baseProfileKey = E.myLocalizedClass .. " - NihilistUI"
	end
end
...
function NI:UpdateProfileKey(overrideKey)
	...
	local lclass =
		isFemale and _G.LOCALIZED_CLASS_NAMES_FEMALE[self.currentClass] or _G.LOCALIZED_CLASS_NAMES_MALE[self.currentClass]
	self.currentLocalizedClass = lclass
	self.profileKey = lclass .. " - " .. (overrideKey and overrideKey or "NihilistUI")
	if not overrideKey then
		self.privateProfileKey = lclass .. " - NihilistUI"
	end
end
...
function NI:SetupSpecProfiles()
	...
	if tbl and type(tbl) == "table" then
		...
		for i, role in ipairs(tbl) do
			_G.ElvDB.namespaces["LibDualSpec-1.0"].char[self.baseProfile][i] = self.currentLocalizedClass .. " - " .. role
		end
	end
end
...
function NI:ShouldInstall()
	...
	if self.db.texture == G.nihilistui.installer.texture and self.db.font == G.nihilistui.installer.font then
		return false
	end
	...
	if not NUI.Lulupeep and (_G.ElvDB.namespaces and _G.ElvDB.namespaces["LibDualSpec-1.0"] and _G.ElvDB.namespaces["LibDualSpec-1.0"].char) then
